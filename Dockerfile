...
RUN apt-get update && \
    apt-get install --no-install-recommends -y wget netcat-traditional procps screen jq moreutils && \
...
WORKDIR /data
RUN wget https://cdn.vintagestory.at/gamefiles/$SERVER_BRANCH/vs_server_linux-x64_$SERVER_VERSION.tar.gz && \
